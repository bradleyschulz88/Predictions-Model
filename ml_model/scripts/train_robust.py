...
def train_with_calibration(X: pd.DataFrame, y: pd.Series) -> dict:
    ...
    # 1. Isotonic
    iso_calibrator = CalibratedClassifierCV(final_model, method="isotonic", cv=5)
    iso_calibrator.fit(X, y)
    
    # 2. Platt (sigmoid)
    platt_calibrator = CalibratedClassifierCV(final_model, method="sigmoid", cv=5)
    platt_calibrator.fit(X, y)
    
    # 3. Custom isotonic on OOF predictions
    iso_reg = IsotonicRegression(out_of_bounds="clip")
    iso_reg.fit(oof_preds, oof_true)
    
    # 4. Platt on OOF (logistic regression)
    platt_lr = LogisticRegression(solver="lbfgs", max_iter=1000)
    platt_lr.fit(oof_preds.reshape(-1, 1), oof_true)
    
    # Evaluate all calibrators on OOF
    calibrators = {
        "isotonic_cv": iso_calibrator,
        "platt_cv": platt_calibrator,
        "isotonic_oof": (iso_reg, "oof"),
        "platt_oof": (platt_lr, "oof"),
    }
    
    best_calibrator = None
    best_brier = float("inf")
    
    print("\nCalibration Comparison (OOF):")
    for name, cal in calibrators.items():
        if name.endswith("_oof"):
            cal_obj, _ = cal
            if name == "isotonic_oof":
                cal_probs = cal_obj.predict(oof_preds)
            else:
                cal_probs = cal_obj.predict_proba(oof_preds.reshape(-1, 1))[:, 1]
        else:
            cal_probs = cal.predict_proba(X)[:, 1]
        
        brier = brier_score_loss(y, cal_probs)
        ll = log_loss(y, cal_probs)
        print(f"  {name}: Brier={brier:.4f}, LogLoss={ll:.4f}")
        
        if brier < best_brier:
            best_brier = brier
            best_calibrator = (name, cal)
    
    print(f"\nBest calibrator: {best_calibrator[0]} (Brier={best_brier:.4f})")
    
    # Final calibration on full data
    if best_calibrator[0].endswith("_cv"):
        final_calibrator = best_calibrator[1]
        final_calibrator.fit(X, y)
    else:
        # Refit on full data
        if best_calibrator[0] == "isotonic_oof":
            final_calibrator = IsotonicRegression(out_of_bounds="clip")
            final_probs = final_model.predict_proba(X)[:, 1]
            final_calibrator.fit(final_probs, y)
        else:
            final_calibrator = LogisticRegression(solver="lbfgs", max_iter=1000)
            final_probs = final_model.predict_proba(X)[:, 1]
            final_calibrator.fit(final_probs.reshape(-1, 1), y)
    ...
    return {
        ...
        "best_cal_name": best_calibrator[0],
...
def save_artifacts(result: dict, X: pd.DataFrame, y: pd.Series):
    ...
    with open(CALIBRATOR_FILE, "wb") as f:
        pickle.dump({
            ...
            "type": result["best_cal_name"],
            "feature_columns": list(X.columns),
        }, f)
    
    metadata = {
        "model_type": "XGBoost",
        "calibration_method": result["best_cal_name"],
        "feature_columns": list(X.columns),
